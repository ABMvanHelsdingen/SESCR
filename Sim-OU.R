...
Ms <- 50; ts <- rep(seq(10,30,length.out=5),each=2)
Cs <- 25; taus <- 0.2; sigmas <- 0.15; epsilon = 0.004
bounds1 <- 0; bounds2 <- 1; bounds3 <- 0; bounds4 <- 1
  
pars <- data.frame(M = Ms, t = ts, C = Cs, beta = betas, sigma = sigmas,
...
for(i in 1:nrow(pars)){
  ...
  centers <- matrix(0, nrow = pars$M[i], ncol = 2)
  centers[,1] <- runif(pars$M[i], bounds[1], bounds[2])
  centers[,2] <- runif(pars$M[i], bounds[3], bounds[4])
  obs <- simOU(pars$M[i], pars$epsilon[i], pars$tau[i], pars$sigma[i], camera_locations, 
                  pars$t[i],a=centers, freq = 100)
  # Save simulation
