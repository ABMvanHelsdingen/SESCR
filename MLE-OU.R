...
file_path = "SimStudy-OU-TMB/"
dir.create(file_path)


pars <- read.csv(paste("OUSCRSims/Pars_",N,".csv",sep=""))
camera_locations = as.matrix(read.csv(paste("OUSCRSims/Cameras_",N,".csv",sep="")))[,2:3]
...
output <- as.data.frame(matrix(0, nrow = nrow(pars), ncol = 29))
names(output) <- c("n_cameras", "M_true", "t", "sigma_true", "beta_true", "epsilon_true",
...
for(i in 1:nrow(pars)){
  ...
  obs = read.csv(paste("OUSCRSims/Sim_Data_",identifier,".csv",sep=""))
  pars <- read.csv(paste("OUSCRSims/Pars_",N,".csv",sep=""))
  ...
  output[i, 5] <- pars$beta[i]
  output[i, 6] <- pars$epsilon[i]
  
  output[i, 7] <- n
  output[i, 8] <- m
  output[i, 9] <- length(unique(cameras))
  
  tab <- table(ids)
  output[i, 10] <- sum(tab == 1)
  output[i, 11] <- max(tab)
  
  TWTable = table(ids, cameras) > 0
  CPI = rowSums(TWTable) # cameras per individual
  
  output[i, 12] <- sum(CPI == 1)
  output[i, 13] <- max(CPI)
  ...
  out_scr <- tryCatch(
    {
      ...
      # Output from secr
      output[i, 14] <- out[2,1] # realised, not expected population
      output[i, 15] <- out[2,2] # se of population
      out1 <- predict(fit)
      output[i, 16] <- (out1[3,2] / 1000) # sigma
      output[i, 17] <- (out1[3,3] / 1000)
      if (!is.na(output[i, 15])){
        output[i, 25] <- 1 # success flag
      }
  ...
  t1 <- Sys.time()
  output[i, 29] <- as.numeric(difftime(t1,t0,units="mins"))
  output[i, 28] <- 4000.20
  
  out_results <- tryCatch(
    {
      output[i, 18] <- coefs[1,1]
      output[i, 19] <- coefs[1,2]
      output[i, 20] <- coefs[4,1]
      output[i, 21] <- coefs[4,2]
      output[i, 22] <- coefs[3,1]
      output[i, 23] <- coefs[2,1]
      output[i, 24] <- coefs[5,1]
    }, error = function(cond){
  ...
  if (!is.na(output[i, 18]) && !is.na(output[i, 14])){
    
    if (abs(output[i, 18] - output[i, 2]) < abs(output[i, 14] - output[i, 2])){
      output[i, 26] <- 1
    }
  }
  
  if (!is.na(output[i, 20]) && !is.na(output[i, 16])){
    if (abs(output[i, 20] - output[i, 4]) < abs(output[i, 16] - output[i, 4])){
      output[i, 27] <- 1
    }
  }
